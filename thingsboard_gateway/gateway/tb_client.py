...
class TBClient(threading.Thread):
    def __init__(self, config, config_folder_path):
        ...
        if credentials.get("accessToken") is not None:
            ...
            self.__pwd = str(credentials["pwd"])
        self.client = TBGatewayMqttClient(self.__host, self.__port, self.__token,self.__pwd, self, quality_of_service=self.__default_quality_of_service)
        if self.__tls:
